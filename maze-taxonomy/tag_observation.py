...
TAGLIST = "./data/maze_taglist.csv"

parser = argparse.ArgumentParser(description='Tagging observation table with tags.csv')
parser.add_argument('-i', '--input', type=str, help='Input observation table', required=True)
parser.add_argument('-t', '--tags', type=str, help='Input tags table', required=True)
parser.add_argument('-m', '--media', type=str, help='Input media table', required=True)
parser.add_argument('-o', '--output', type=str, help='Output observation table', required=True)
...
df_tags = df_tags.select([
    pl.col("filename").str.split_exact(".", 1).struct.rename_fields(["fileName", "extension"]),
    "species",
    "individual",
    pl.col("rating").replace("", "0"),
]).unnest("filename")
df_taglist = pl.read_csv(TAGLIST).select([
    pl.col("tag").alias("species"),
    "scientificName"
])
...
df_observations_tagged = df_observations_tagged.select([
    ...
    pl.concat_str(pl.lit("eng: "), pl.col("species"), pl.lit("|Rating:"), pl.col("rating")).alias("observationTags"),
