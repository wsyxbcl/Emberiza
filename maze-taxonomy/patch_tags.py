...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Patch tags.csv with tagdict and taglist')
    parser.add_argument('tags', type=str, help='Input tags csv file')
    parser.add_argument('-o', '--output', type=str, help='Output (patched) tags csv file', required=True)
    ...
    df_maze_tagdict_concat = df_maze_tagdict.select([
        ...
        pl.concat_list(df_maze_tagdict.columns[1:]).alias("synonym").list.drop_nulls()
    ])
