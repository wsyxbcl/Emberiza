        ...
        for collection in data['collections']:
            for deployment in collection['deployments']:
                for resource in deployment['resources']:
                    trapper_datetime = datetime.fromisoformat(resource['date_recorded']).astimezone(ZoneInfo("Asia/Shanghai")).strftime("%Y-%m-%d %H:%M:%S")
                    writer.writerow([collection['name'], deployment['deployment_id'], resource['file'], trapper_datetime])
