...
def extract_individual_info(yaml_files, favorite_mode):
    ...
    def count_body_part(individual, keywords):
        if "模糊" in keywords:
            ...
            for keyword in keywords:
                if (keyword in body_parts_count) or (
                    keyword in blurred_body_parts_count
                ):
    ...
    for yaml_file in yaml_files:
        ...
        # anaylze keywords
        keywords = [
            keyword.strip() for keyword in data["Details"]["Keywords"].split(",")
        ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract individual info from linchong photoprism sidecar files (yaml)"
    )
    parser.add_argument("directory")
    parser.add_argument("output_file")
    parser.add_argument(
        "--favorite-only", help="Only count favorite images", action="store_true"
    )
