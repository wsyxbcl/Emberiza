...
if USE_CSV:
    ...
    SPECIES_RATIO = dict(zip(df_species_ratio["species"].to_list(), df_species_ratio["ratio"].to_list()))
...
for species, ratio in SPECIES_RATIO.items():
    ...
    print(f"Species: {species}, Aimed: {num_resource}, Sampled: {len(df_species_sample)}/{len(df_species)}")
